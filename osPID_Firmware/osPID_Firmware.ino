...
/*-----( Declare myLCD Constants )-----*/
#define I2C_ADDR    0x20  // Define I2C Address for the PCF8574A 
//---(Following are the PCF8574 pin assignments to myLCD connections )----
// This are different than earlier/different I2C myLCD displays
...
// #kls# LiquidCrystal_I2C  myLCD(I2C_ADDR,En_pin,Rw_pin,Rs_pin,D4_pin,D5_pin,D6_pin,D7_pin);
...
byte ctrlDirection = DIRECT; // 0 = Direct
byte modeIndex = MANUAL; // 0 = Manual; 1 = Auto
...
void setup()
{
    ...
    Serial.println("writing copywrite notice");
    myLCD.print(" osPID   ");
    myLCD.write(13);
    myLCD.print(" v1.60   ");
    
    Serial.println("sending note");
    myLCD.write(210);                // Quarter note
    myLCD.write(220);                // A tone
    myLCD.write(222);                // A tone
...
void drawmyLCD()
{
    ...
    if(editing) setCursor(editDepth, highlightFirst?0:1);
}
...
void drawItem(byte row, boolean highlight, byte index)
{
    ...
    case TYPE_NAV:
        ...
        case 1: 
            ...
            break;
        case 2: 
            myLCD.print(tuning ? "Cancel " : "ATune  "); 
            break;
        case 3:
            myLCD.print(runningProfile ? "Cancel " : profname);
            break;
        default: 
            return;
        }
    ...
    case TYPE_VAL:
        ...
        myLCD.print(edit? '[' : (highlight ? (canEdit ? '>':'|') : 
        ' '));

        if(isnan(val))
        { //display an error
            myLCD.print(icon);
            myLCD.print( now % 2000<1000 ? " Error":"      "); 
            return;
        }
    ...
    case TYPE_OPT: 
        ...
        myLCD.print(edit ? '[': (highlight? '>':' '));    
        switch(index)
        {
        case 7:    
            myLCD.print(modeIndex==0 ? "M Man  ":"M Auto "); 
            break;
        case 11://12: 

            myLCD.print(ctrlDirection==0 ? "A Direc":"A Rever"); 
            break;
        }
...
{
    byte row0Cols[] = {128,129,130,131,132,133,134,135,136,137,138,139,140,141,142,143};
    byte row1Cols[] = {148,149,150,151,152,153,154,155,156,157,158,159,160,161,162,163};
    byte *rowCols[] = {row0Cols, row1Cols};
    myLCD.write(rowCols[row][col]);
}
...
void ok()
{
    ...
    else
    {
        ...
        case 0: 
            ...
            changeflag = false;
            // Serial.println("ok function - delay 5 seconds");
            delay(5);
            break;
        case 1: 
            curMenu=2;
...
void ProfileRunTime()
{
    ...
    {
        // if the direction is reverse, change the sign
        float err = (input-setpoint) * (ctrlDirection == REVERSE ? -1 : 1);
        if(helperflag) //we're just looking for a cross
        {
            float curHelperVal = (ctrlDirection == REVERSE ? -1 : 1) * helperVal;
            if(curHelperVal > 0 || err > 0)
            {
                gotonext = true;
            }
        }
    ...
    else if(curType == 127) //buzz
    {
        if(now < helperTime){
            ...
            myLCD.write(214);                // Quarter note
            //myLCD.write(215);                // 3rd scale 220 Hz
            myLCD.write(220);                // A tone
            myLCD.write(220);                // A tone
            myLCD.write(220);                // A tone
        }
