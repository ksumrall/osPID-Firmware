...
const byte thermocoupleSO = 12;
const byte thermocoupleCLK = 13;
byte inputType = 0; // 0 = thermocouple; 1 = thermistor
double THERMISTORNOMINAL = 10;
double BCOEFFICIENT = 1;
double TEMPERATURENOMINAL = 293.15;
double REFERENCE_RESISTANCE = 10;
...
double ReadInputFromCard()
{
    ...
    float temperature = (voltage - 1.25) / .005;
    return temperature;
}
...
void WriteToOutputCard(double value)
{
    ...
    */
    if(outputType == 0){
        ...
        /*
        Serial.print("WindowSize value = ");
        Serial.println(WindowSize);
        Serial.print("wind value = ");
        Serial.println(wind);
        Serial.print("oVal value = ");
        Serial.println(oVal);
        Serial.print("(oVal>wind) value = ");
        Serial.println((oVal>wind) ? "True" : "False");
        */
